...
@section('page-subtitle', 'Modifier le service')

@section('toolbar-actions')
<div class="d-flex align-items-center py-1">
    <a href="{{ route('admin.luxury.services.show', $service) }}" class="btn btn-sm btn-light me-3">
...
<form action="{{ route('admin.luxury.services.update', $service) }}" method="POST" enctype="multipart/form-data" id="kt_services_edit_form">
    @csrf
    @method('PUT')
                ...
                <div class="card-body pt-0">
                    <div class="mb-7">
                        ...
                        <input type="text" class="form-control form-control-solid @error('nom') is-invalid @enderror"
                               name="nom" value="{{ old('nom', $service->nom) }}" placeholder="Ex: Transport privé en limousine"/>
                        @error('nom')
                    ...
                    <div class="mb-7">
                        ...
                        <select class="form-select form-select-solid @error('categorie') is-invalid @enderror"
                            ...
                            @foreach($categories as $key => $label)
                                <option value="{{ $key }}" {{ old('categorie', $service->categorie) == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-7">
                        ...
                        <textarea class="form-control form-control-solid @error('description') is-invalid @enderror"
                                  name="description" rows="5" placeholder="Décrivez votre service de luxe en détail...">{{ old('description', $service->description) }}</textarea>
                ...
                <div class="card-body pt-0">
                    <div class="mb-7">
                        ...
                        <select class="form-select form-select-solid @error('type_prix') is-invalid @enderror"
                            ...
                            @foreach($typesPrix as $key => $label)
                                <option value="{{ $key }}" {{ old('type_prix', $service->type_prix) == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-7" id="prix_base_container">
                        ...
                        <input type="number" class="form-control form-control-solid @error('prix_base') is-invalid @enderror"
                               name="prix_base" value="{{ old('prix_base', $service->prix_base) }}" placeholder="0.00" min="0" step="0.01"/>
            ...
            <div class="card mb-5 mb-xl-8">
                ...
                <div class="card-body pt-0">
                    ...
                    <div id="options-container">
                        @php
                            $options = old('options_disponibles', $service->options_disponibles ?? []);
                        @endphp
                    ...
                    <div id="no-options-message" class="text-center text-muted py-5" style="{{ ($options && count($options) > 0) ? 'display:none;' : '' }}">
                ...
                <div class="card-body pt-0">
                    <div class="mb-7">
                        ...
                        <input type="text" class="form-control form-control-solid @error('fournisseur') is-invalid @enderror"
                               name="fournisseur" value="{{ old('fournisseur', $service->fournisseur) }}" placeholder="Ex: Prestige Limousines Paris"/>
                        @error('fournisseur')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-7">
                        ...
                        <input type="text" class="form-control form-control-solid @error('contact_fournisseur') is-invalid @enderror"
                               name="contact_fournisseur" value="{{ old('contact_fournisseur', $service->contact_fournisseur) }}"
        ...
        <div class="col-xl-4">
            ...
            <div class="card mb-5 mb-xl-8">
                ...
                <div class="card-body pt-0">
                    <div class="mb-7">
                        ...
                        <div class="form-check form-switch form-check-custom form-check-solid">
                            <input class="form-check-input" type="checkbox" name="actif" value="1"
                                   id="service_actif" {{ old('actif', $service->actif) ? 'checked' : '' }}/>
                    ...
                    <div class="mb-7">
                        <label class="fs-6 fw-bold mb-2">Image actuelle</label>
                        <div class="image-input image-input-outline">
                            <div class="image-input-wrapper w-125px h-125px" style="background-image: url('{{ $service->image_url }}')"></div>
                        </div>
                    </div>
                    @endif

                    <div class="mb-7">
                        <label class="fs-6 fw-bold mb-2">{{ $service->image ? 'Changer l\'image' : 'Image du service' }}</label>
            ...
            <div class="card bg-light-info mb-5">
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        <span class="text-gray-600 fw-bold">ID:</span>
                        <span class="text-gray-800 fw-bolder">#{{ $service->id }}</span>
                    </div>
                    <div class="mb-3">
                        <span class="text-gray-600 fw-bold">Créé le:</span>
                        <span class="text-gray-800">{{ $service->created_at->format('d/m/Y') }}</span>
                    </div>
                    <div>
                        <span class="text-gray-600 fw-bold">Modifié le:</span>
                        <span class="text-gray-800">{{ $service->updated_at->format('d/m/Y') }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="d-flex justify-content-end mt-8">
        <a href="{{ route('admin.luxury.services.show', $service) }}" class="btn btn-light me-3">Annuler</a>
        <button type="submit" class="btn btn-primary" id="submit_button">
            <span class="indicator-label">Enregistrer les modifications</span>
            <span class="indicator-progress">Enregistrement...
...
document.addEventListener('DOMContentLoaded', function() {
    let optionIndex = {{ count(old('options_disponibles', $service->options_disponibles ?? [])) }};
